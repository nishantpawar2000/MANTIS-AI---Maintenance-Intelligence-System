...
function createSlide(pres, theme) {
  ...
  slide.addText("08  /  TECH STACK", {
    x: 0.5, y: 0.35, w: 9, h: 0.3,
    fontSize: 11, fontFace: "Arial", color: theme.accent, bold: true,
    charSpacing: 4, margin: 0
  });

  slide.addText("Built lean, runs anywhere in your network.", {
    x: 0.5, y: 0.7, w: 9, h: 0.5,
    fontSize: 24, fontFace: "Arial", bold: true, color: theme.primary, margin: 0
  });
  slide.addText("Zero external dependencies for the demo. Single Node process, no database, no vendor lock-in.", {
    x: 0.5, y: 1.25, w: 9, h: 0.3,
    fontSize: 12, fontFace: "Arial", color: theme.secondary, margin: 0
  });

  // Layered stack: 4 horizontal layers
  const layers = [
    { name: "Frontend",            items: "Vanilla HTML / CSS / JS  -  dependency-free dashboard  -  SVG architecture diagram",  color: theme.accent },
    ...
    { name: "Plant systems (target)", items: "PI Historian  -  SCADA alarms  -  CMMS  -  SOPs / manuals  -  Work orders",     color: "475569" }
  ];
  ...
  layers.forEach((l, i) => {
    ...
    // Square corner fix: cover right side of badge so it stays rounded only on left
    slide.addShape(pres.shapes.RECTANGLE, {
  ...
  slide.addText("All five AI agents run in a single process. Frontend remains responsive even when the LLM is unreachable.", {
